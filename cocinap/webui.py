...
class _Handler(BaseHTTPRequestHandler):
    ...
    def _stream_mjpeg(self, webui):
        ...
        while not webui._stopped:
            ...
            if frame is not None:
                # Downscale for mobile to reduce bandwidth and encode time
                h, w = frame.shape[:2]
                if w > 480:
                    scale = 480.0 / w
    ...
    def do_POST(self):
        ...
        if path == "/api/config" and webui:
            ...
            try:
                data = json.loads(raw)
                minutes = int(data.get("minutes", cfg.KITCHEN_TIMER_DURATION))
                webui.engine.timer_start(minutes)
...
class WebUI:
    ...
    def send_fcm_event(self, title, body, alarm_type, severity):
        """Send a one-off push notification (e.g. timer expired)."""
        if not cfg.ENABLE_FCM or not cfg.FCM_TOKENS:
            return
        if not _HAS_FCM:
            return
