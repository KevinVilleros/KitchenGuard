...
class RiskAnalyzer:
    ...
    def reset_session(self):
        """Reset person tracking when a new timer window starts."""
        self.last_person_time = time.time()
        self.consecutive_no_person = 0
        self.consecutive_yes_person = 0
    ...
    def analyze(self, detections, timer_active=False, timer_alert_seconds=None,
                alarm_unattended=True, alarm_smoke=True, alarm_fire=True):
        current_time = time.time()
        alerts = []
        if timer_alert_seconds is None:
            timer_alert_seconds = KITCHEN_TIMER_ALERT_SECONDS
        ...
        # ---- Timer mode: unattended alert if person leaves during cooking ----
        if timer_active and not has_person and n_pots > 0:
            person_absent = current_time - self.last_person_time
            if person_absent >= timer_alert_seconds and alarm_unattended:
                alerts.append({
                    "type": "TIMER_OLVIDO", "severity": "ALTO",
                    "message": f"⏰ Nadie en cocina! {int(person_absent)}s sin supervisión",
                    "confidence": min(person_absent / (timer_alert_seconds * 2), 0.9),
                })

        # ---- Fire alarm toggle ----
        if not alarm_fire:
            pass  # skip fire alerts below
        elif fire_in_stove and not has_person and valid_fire_now:
        ...
        if n_pots > 0 and not has_person and alarm_unattended:
    ...
    def should_trigger_alarm(self, alerts):
        ...
        # TIMER_OLVIDO → unattended alarm
        for a in alerts:
            if a["type"] == "TIMER_OLVIDO":
