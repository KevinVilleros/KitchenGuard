...
class ApiService {
  ...
  Future<void> timerStop() async {
    await http.post(Uri.parse("$_baseUrl/api/timer/stop"))
        .timeout(const Duration(seconds: 3));
  }

  Future<void> timerPause() async {
    await http.post(Uri.parse("$_baseUrl/api/timer/pause"))
        .timeout(const Duration(seconds: 3));
  }

  Future<void> timerResume() async {
    await http.post(Uri.parse("$_baseUrl/api/timer/resume"))
        .timeout(const Duration(seconds: 3));
  }
}
